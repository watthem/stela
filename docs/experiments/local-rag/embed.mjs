...
//   node embed.mjs [--kind parent|sentence] [--limit N] [--batch 16]
import { createHash } from 'node:crypto';
import { MODEL, embed, pool, toVec } from './db.mjs';
...
while (done < limit) {
  const { rows } = await pool.query(
    `SELECT id, embed_text FROM chunks WHERE embedding IS NULL ${kind ? 'AND kind=$2' : ''}
...
const left = (await pool.query('SELECT count(*)::int n FROM chunks WHERE embedding IS NULL')).rows[0].n;
